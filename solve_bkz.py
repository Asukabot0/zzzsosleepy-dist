...
# Check if fpylll is available, if not, try importing it from the other location
try:
    from fpylll import IntegerMatrix, LLL, BKZ
except ImportError:
    # try running directly via appropriate python path if needed (this script won't fix path)
    # assume executed by correct python
    from fpylll import IntegerMatrix, LLL, BKZ
...
def solve(seed: bytes, outs: List[int]) -> bytes:
    ...
    print("[*] Running LLL reduction (safe delta)...")
    LLL.reduction(M, delta=0.75)
    print("[*] LLL done. Checking basis vectors...")
    ...
    for i in range(dim):
        ...
        if key is not None:
            print(f"[+] Key found at index {i} after LLL!")
            return key

    print("[*] LLL failed. Running BKZ-20...")
    BKZ.reduction(M, BKZ.Param(block_size=20))
    print("[*] BKZ-20 done. Checking basis vectors...")

    for i in range(dim):
        vec = [int(M[i, j]) for j in range(dim)]
        key = try_extract_key(vec, A_unsigned, outs)
        if key is not None:
            print(f"[+] Key found at index {i} after BKZ-20!")
            return key

    print("[*] BKZ-20 failed. Running BKZ-30 (might be slow)...")
    BKZ.reduction(M, BKZ.Param(block_size=30))
    for i in range(dim):
        vec = [int(M[i, j]) for j in range(dim)]
        key = try_extract_key(vec, A_unsigned, outs)
        if key is not None:
            print(f"[+] Key found at index {i} after BKZ-30!")
            return key

    raise RuntimeError("key not found even after BKZ-30")
